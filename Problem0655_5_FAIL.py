...
class Palindrome:
  ...
  def __iter__(self):
    ...
    self.factors_int = int(''.join([str(i) for i in self.factors]))
    self.setValue()
    return self

  def incrementFactors(self):
    self.factors_int = self.factors_int + 1
    self.factors = [int(i) for i in str(self.factors_int)]
    
  def setValue(self):
    if self.odd :
      self.value = int(''.join([str(i) for i in self.factors + self.factors[-1::-1] ]))
    else:
      self.value = int(''.join([str(i) for i in self.factors[:] + self.factors[-2::-1] ]))
